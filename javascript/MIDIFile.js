...
MIDIFile.prototype.getEvents = function(type=[], subtype=[], ignoredTracks=[], mergeTracks=0) {
  var events;
  var event;
  var playTime = 0;
  var filteredEvents = [[]];
  ...
  if (1 !== format || 1 === this.tracks.length) {
    for (i = 0, j = this.tracks.length; i < j; i++) {
		if(!ignoredTracks.includes(i)&&ignoredTracks.length!=0) continue;
		if (!mergeTracks) filteredEvents[i]=[];
    ...
    do {
      ...
      if (-1 !== smallestDelta) {
        ...
        if (
          ...
          filteredEvents[mergeTracks?0:event.track].push(event);
        }
        // getting next event
