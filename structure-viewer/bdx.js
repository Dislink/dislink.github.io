(function (){
	...
	DataView.prototype.getNBTUncompressed = function (offset=0, littleEndian=true){
		// nbt.Reader has no offset parameter — slice from offset so parsing
		// starts at the NBT blob instead of buffer start (fixes "Top tag
		// should be a compound" on BDX files with command 41).
		let reader=new nbt.Reader(this.buffer.slice(offset), littleEndian);
